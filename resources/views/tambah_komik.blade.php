    ...
    <div class="btn text-center mb-3">
        ...
        <div class="max-w-xl mx-auto bg-white rounded-md overflow-hidden shadow-lg mt-8">
            <div class="px-6 py-4">
                ...
                <form method="POST" action="/proses_tambah_komik" enctype="multipart/form-data">
                @csrf
                    <div class="mt-3">
                        <label for="judul" class="block text-sm font-medium text-gray-700">Judul</label>
                        <input type="text" id="judul" name="judul" class="mt-1 p-2 w-full border rounded-md" value="{{ old('judul') }}">
                        @error('judul')
                            <p class="text-red-500">{{ $message }}</p>
                        @enderror
                        <label for="penulis" class="block mt-4 text-sm font-medium text-gray-700">Penulis</label>
                        <input type="text" id="penulis" name="penulis" class="mt-1 p-2 w-full border rounded-md" value="{{ old('penulis') }}">
                        @error('penulis')
                            <p class="text-red-500">{{ $message }}</p>
                        @enderror
                        ...
                        <label for="file-upload" class="cursor-pointer">
                            ...
                            <div id="drop-area" class="mt-2 flex justify-center rounded-lg border border-dashed border-gray-900/25 px-6 py-10 ">
                                <div class="text-center" >
                                    <img id="preview-image" src="{{ url('img/profile_kosong.jpg') }}" width="200" alt="">
